...
import { tool } from "ai";
import { z } from "zod";
import { ApiRequestError, createReturn, getCategories, getOrder, getProducts, preauthorizeRefund, notifyReturnInProcess,
    getProductById,getProductStock, getBackOfficeReturns, 
    getBackOfficeSales, 
    getBackOfficeStock, 
...
export const searchProducts = tool({
  ...
  export const getProductDetails = tool({
    description: `Get the full details for ONE specific product the user is asking about (e.g. "tell me more about the black hoodie", "what sizes does the tote bag come in?", "is the water bottle in stock?"). Use this instead of searchProducts whenever the user has already named or clearly identified a single item and wants more than the summary — full description, all images, category, tags, and live stock/availability. If you don't yet know which product they mean, call searchProducts first (or ask them to clarify) to get the product's id or slug, then call getProductDetails with that id/slug for the deep-dive answer. Do not use this for browsing, comparing multiple items, or open-ended recommendations — that's what searchProducts is for.`,
    inputSchema: z.object({
      idOrSlug: z
        .string()
        .describe(
          "The product's id or slug, e.g. from a prior searchProducts result. Slugs are human-readable like 'classic-black-hoodie'.",
        ),
    }),
    execute: async ({ idOrSlug }) => {
      ...
      try {
        ...
        try {
          const stockInfo = await getProductStock(idOrSlug);
          stock = {
          ...
          console.error("[getProductDetails] stock lookup failed:", stockErr);
        }
